...
def run(args):
    ...
    agent_config = {'model_lr': args.model_lr, 'momentum': args.momentum, 'model_weight_decay': args.model_weight_decay,
                    ...
                    'owm_lr':args.owm_lr,
                    'bn_lr': args.bn_lr,
                    'svd_lam': args.svd_lam,
                    'svd_last': args.svd_last
                    }

    # agent = agents.__dict__[args.agent_type].__dict__[args.agent_name](agent_config)
    agent = factory('owm_agent', args.agent_type, args.agent_name)(agent_config)
    ...
    if args.offline_training:  # Non-incremental learning / offline_training / measure the upper-bound performance
        ...
        for i in range(len(task_names)):
            ...
            # Learn
            agent.train_task(train_loader, train_name, val_loader)    
            ...
            for j in range(i + 1):
                ...
                train_loader = torch.utils.data.DataLoader(train_data,
                                                         batch_size=args.batch_size, shuffle=False,
                                                         num_workers=args.workers)
                acc_table_train[val_name][train_name] = agent.validation(train_loader)
                print("**************************************************")
...
def get_args(argv):
    ...
    parser.add_argument('--agent_type', type=str, default='owm', help="The type (filename) of agent")
    parser.add_argument('--agent_name', type=str, default='owm_based', help="The class name of agent")
    ...
    parser.add_argument('--owm_lr', type=float, default=0.0005, help="Classifier Learning rate")
    parser.add_argument('--bn_lr', type=float, default=0.0005, help="Classifier Learning rate")

    parser.add_argument('--svd_lam', type=float, default=0,help='combination gradient')
    parser.add_argument('--svd_last', type=float, default=1.0, help='reserve eigenvector')

    parser.add_argument('--momentum', type=float, default=0)

    parser.add_argument('--model_weight_decay', type=float, default=1e-5) # 1e-4
