...
class Adam(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8, svd=False, thres=1.001, weight_decay=0, amsgrad=False):
        if not 0.0 <= lr:
    ...
    def get_transforms(self):
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                dim = self.eigens[p]['eigen_value'].shape[0]
                ...
                #transform = torch.eye(basis.shape[0]).to(device)
                #exit()
                #lamda = 1
                #transform = torch.eye(dim).to(device) + lamda * torch.mm(basis, basis.transpose(1, 0))
                self.transforms[p] = transform / torch.norm(transform)
                self.transforms[p].detach_()

    def get_eigens(self, fea_in,train_name):
        for group in self.param_groups:
            svd = group['svd']
            if svd is False:
                continue
            for p in group['params']:
                ...
                device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
                #dim = self.eigens[p]['eigen_value'].shape[0]
                #print('task_names:',train_name)
                #print(type(train_name))
                lamda = 0.0015
                #lamda = 0.00745
                _, eigen_value, eigen_vector = torch.svd(lamda * fea_in[p] + torch.eye(fea_in[p].size(0)).to(device), some=False)
